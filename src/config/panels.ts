...
export const VARIANT_PANEL_OVERRIDES: Partial<Record<string, Partial<Record<string, Partial<PanelConfig>>>>> = {
  ...
  'fx-7ccy': {
    map:              { name: 'グローバルマーケットマップ' },
    'live-news':      { name: 'FX ヘッドライン' },
    forex:            { name: '為替・通貨' },
    centralbanks:     { name: '中央銀行ウォッチ' },
    'yield-curve':    { name: 'イールドカーブ' },
    'macro-signals':  { name: 'マーケットレーダー' },
    economic:         { name: '経済指標' },
    'economic-calendar': { name: '経済カレンダー' },
    'cot-positioning':   { name: 'COT ポジション' },
    bonds:            { name: '債券・金利' },
    'fear-greed':     { name: '恐怖＆貪欲指数' },
    'market-breadth': { name: '市場の広がり' },
    derivatives:      { name: 'デリバティブ・オプション' },
    'markets-news':   { name: 'マーケットニュース' },
    'economic-news':  { name: '経済ニュース' },
    monitors:         { name: 'マイモニター' },
  },
};
