...
export const VARIANT_PANEL_OVERRIDES: Partial<Record<string, Partial<Record<string, Partial<PanelConfig>>>>> = {
  ...
  'fx-7ccy': {
    'fx-ai-insights':    { name: 'FX AI インサイト', enabled: true },
    'fx-volatility':     { name: 'FX ボラティリティ＆レンジ', enabled: true },
    'fx-positioning':    { name: 'FX COT ポジション', enabled: true },
    'fx-policy-rates':   { name: '政策金利 (7通貨)', enabled: true },
    'fx-calendar':       { name: 'FX 経済カレンダー', enabled: true },
    'macro-signals':     { name: 'マーケットレーダー', enabled: true },
    map:                 { name: 'グローバルマーケットマップ', enabled: false },
    'live-news':         { name: 'FX ヘッドライン', enabled: false },
    forex:               { name: '為替・通貨', enabled: false },
    centralbanks:        { name: '中央銀行ウォッチ', enabled: false },
    'yield-curve':       { name: 'イールドカーブ', enabled: false },
    economic:            { name: '経済指標', enabled: false },
    'economic-calendar': { name: '経済カレンダー', enabled: false },
    'cot-positioning':   { name: 'COT ポジション', enabled: false },
    bonds:               { name: '債券・金利', enabled: false },
    'fear-greed':        { name: '恐怖＆貪欲指数', enabled: false },
    'market-breadth':    { name: '市場の広がり', enabled: false },
    derivatives:         { name: 'デリバティブ・オプション', enabled: false },
    'markets-news':      { name: 'マーケットニュース', enabled: false },
    'economic-news':     { name: '経済ニュース', enabled: false },
    monitors:            { name: 'マイモニター', enabled: false },
  },
};
